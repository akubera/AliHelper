...
parser.add_argument('-t','--time', help='Time (in minutes) to repeat the action', default='30', type=float)
...
def kill_job(jid):
  ...
  while True:
    info = yield from asyncio.create_subprocess_shell(info_cmd, stdout=PIPE, stderr=PIPE)
    ...
    if 'DONE' not in stats:
      ...
      kill = yield from asyncio.create_subprocess_shell(kill_cmd, stdout=PIPE, stderr=PIPE)
      jobs_killed = []
      while True:
...
#       data = yield from kill_ret
    #  proc = asyncio.create_subprocess_exec(*aliargs, stdout = asyncio.subprocess.PIPE)
    # proc = asynco.create_subprocess_shell(' '.join(aliargs), stdout = asyncio.subprocess.PIPE)
#     res = yield from proc
